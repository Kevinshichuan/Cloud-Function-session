...
const LOCATION = "us-central1"; // Replace with the location of Cloud Tasks queue
...
export const scheduleSessionExpiry = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    } else if (beforeSession && !afterSession) {
      // if it goes in this statement -> it always has a beforeSession (user has to already created the task before)
...
/*
    ...
    Intuition:
        1. When the session is updated and we schedule a task to clear it after a certain duration, 
        create unique task identifier based on the session ID -> allow to associate the task with the specific session
        
        2. Store the unique task identifier in the Firestore document associated with the session

        3. When schedule a new task for the session, check if the session still exists in Firestore
         before scheduling the task. If the session has been deleted, do not schedule a new task

        4. To cancel a scheduled task, we can use the Cloud Tasks client to delete it based on the
         unique task identifier that we created on step one. This ensures that even if a session is
          deleted, any previously scheduled tasks associated with it can be canceled.
*/
...
async function storeTaskIdentifierInFireStore(
  sessionId: any,
  userId: any,
  taskIdentifier: any
) {
